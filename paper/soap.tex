...
compiler. The new class loader loaded a Scene that had 58\% to 64\% less classes,
representing memory savings of 56\% to 82\%.
\end{abstract}
...
The contents are organized as follows. In section \ref{sec:soot-cl}, we will first describe the current Soot class loading algorithm and why it has problems with memory usage. Then, in section \ref{sec:new-cl}, we will describe our methodology and describe the new class loading algorithm in detail. In section \ref{sec:api}, we describe the new API added to Soot. We then compare the performance of our method with Soot's class loading in \rb in section \ref{sec:eval}. Finally, we conclude in section \ref{sec:conclusion}.
...
This is handled by a backward traversal from the entry points. When traversing the methods, we see if a method body makes a call into the \emph{reachable set}. If it does, the edge is added to the call graph and the new method is added to the forward traversal queue.
...
Our test cases include our running example, and the SPECjvm2008 benchmark \cite{SPEC:paper}. We measured the execution time with Soot's built-in feature, and used the Classmexer\footnote{\url{http://www.javamex.com/classmexer/}} agent to measure the memory used by the loaded \sootclass objects. The overall memory usage was calculated using the JVM's global memory usage feature.
...
SPEC jvm 2008 (Soot) & 6539 & 3411 & 333 & 469 \\\hline
SPEC jvm 2008 (RBC) & 2737 & 3733 & 186 & 337 \\\hline
...
We notice a significant decrease in the number of classes loaded, with a decrease in processing time for the large test casse. While we were careful about performance in our implementation, it is possible that further optimizations would allow us to improve our loading time for the small test case.
...
We have shown data structures and algorithms to efficiently load a Soot Scene. The new class loader loaded a Scene that had 58\% to 64\% less classes, representing memory savings of 56\% to 82\%.
...
\bibitem[Bacon and Sweeney(1996)]{BaconRTA}
...
\bibitem[Dean et~al.(1995)Dean, Grove, and Chambers]{deanetal}
...
\bibitem[Lam et~al.(2011)Lam, Bodden, Lhot{\'a}k, and Hendren]{soot-retro}
...
\bibitem[Shiv et~al.(2009)Shiv, Chow, Wang, and Petrochenko]{SPEC:paper}
K.~Shiv, K.~Chow, Y.~Wang, and D.~Petrochenko.
\newblock {SPECjvm2008} performance characterization.
\newblock In \emph{Proceedings of the 2009 SPEC Benchmark Workshop on Computer
  Performance Evaluation and Benchmarking}, pages 17--35. Springer-Verlag,
  2009.
\newblock ISBN 978-3-540-93798-2.

\bibitem[Sundaresan et~al.(2000)Sundaresan, Hendren, Razafimahefa,
  Vall{\'e}e-Rai, Lam, Gagnon, and Godin]{Sundaresan:2000:PVM:354222.353189}
...
\newblock \emph{SIGPLAN Not.}, 35\penalty0 (10):\penalty0 264--280, Oct. 2000.
\newblock ISSN 0362-1340.

\bibitem[Umanee(2006)]{shimple}
N.~Umanee.
\newblock Shimple: An investigation of static single assignment form.
\newblock Master's thesis, McGill University, 2006.

\bibitem[Vall{\'e}e-Rai and Hendren(1998)]{jimpletr}
