  ...
  public static void main(String[] args){
    ...
    PackManager.v().getPack("wjtp").add(new Transform("wjtp.stats", new SceneTransformer() {
      ...
      protected void internalTransform(String phaseName, Map options) {
        ...
        G.v().out.println("Memory used in wjtp phase: " + memoryUsed / (float) FileUtils.ONE_MB + " Mb\tMemory used for SootClass objects: "
                + memoryUsedClasses / (float)FileUtils.ONE_MB + " Mb");

      }
    }));

    soot.Main.main(new String[]{
           "-time", "-w","-prepend-classpath", "-output-format", "none", "-soot-classpath", args[0], "A"
    });
  }
}
